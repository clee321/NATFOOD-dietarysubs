...
foreach file in `files'{
	...
	**#%Difference
		svy, subpop (if subpop==1 & sub_flag==1): mean (perc_HEI2015*)
...
foreach file in `files'{
	...
	**#AgeCat
	forval f=1/8 {
		...
		//%difference	
		svy, subpop (if subpop==1 & sub_flag==1 & ageCat==`f'): mean (perc_HEI2015*)
	...
	**#genderCat
	forval f=1/2 {
		...
		//%difference
		svy, subpop (if subpop==1 & sub_flag==1 & genderCat==`f'): mean (perc_HEI2015*)
...
foreach file in `files'{
	...
	**#raceCat //5 race/ethn categories
	forval f=1/5 {
		...
		//%difference
		svy, subpop (if subpop==1 & sub_flag==1 & raceCat==`f'): mean (perc_HEI2015*)
