...
*Regression - Paired t-test
	...
	*	svy, subpop (if subpop==1): reg `var'
	*}
...
foreach file in `files'{
	...
	**#eduCat
	forval f=1/4 {
		...
		//original
		svy, subpop (if subpop==1 & eduCat==`f'): mean (ttl_origrams-ttl_orisele ttl_orighgcons)
		...
		putexcel set "$Results/2015-16/Mean_nutri_ghg/Overall pop/`filename'_mean_nutri_ghg_seqn_bydemo.xlsx", sheet ("eduCat_`f'") modify
		...
		//substituted
		svy, subpop (if subpop==1 & eduCat==`f'): mean (ttl_subgrams-ttl_subsele ttl_subghgcons)
		...
		//difference
		svy, subpop (if subpop==1 & eduCat==`f'): mean (diff_grams-diff_ghgcons)
...
foreach file in `files'{
	...
	**#pirCat
	foreach f of numlist 1/4 99 {
		...
		//original
		svy, subpop (if subpop==1 & pirCat==`f'): mean (ttl_origrams-ttl_orisele ttl_orighgcons)
		...
		putexcel set "$Results/2015-16/Mean_nutri_ghg/Overall pop/`filename'_mean_nutri_ghg_seqn_bydemo.xlsx", sheet ("pirCat_`f'") modify
		...
		//substituted
		svy, subpop (if subpop==1 & pirCat==`f'): mean (ttl_subgrams-ttl_subsele ttl_subghgcons)
		...
		//difference
		svy, subpop (if subpop==1 & pirCat==`f'): mean (diff_grams-diff_ghgcons)
