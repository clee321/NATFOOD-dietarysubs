*HEI Scores Calculation - Substituted dataset
...
rename sub_FOODCODE sub_code ///rename to sub_code to merge with sub data
*no duplicates on FOODCODE
...
use "$Analysis/Substitution/2015-16/5_OrixSub_dataset/Protein_Sub_dataset_high_origxsub.dta", clear
...
*        from using                      3,605  (_merge_FPED==2)
*
*    Matched                           121,481  (_merge_FPED==3)
*    -----------------------------------------
...
*1,048 missing values - records of "Milk, human" without grams
...
///Only drop seqn after second aggregation otherwise sub info won't represent the total but just one food item.
...
save "$Analysis/HEI/Intermediate/NHANES_DEMO_GHG_FPED_for HEI calc_prot hi_allsubpop.dta", replace
  
********
use "$Analysis/HEI/Intermediate/NHANES_DEMO_GHG_FPED_for HEI calc_prot hi_allsubpop.dta", clear
...
*-------------+---------------------------------------------------------
*HEI2015_TO~E |      8,505    49.21242    14.25049          0   94.35164
...
*    Variable |        Obs        Mean    Std. dev.       Min        Max
*-------------+---------------------------------------------------------
*sHEI2015_T~E |      8,505    49.40366    14.25856          0   94.35164
...
save "$Analysis/HEI/2015-16 HEI Total Score per seqn_prot high_allpop.dta", replace
