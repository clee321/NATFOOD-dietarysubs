...
* Summary table with GHG/100g of Food codes (showing food description, categ. description and level 1 food categories) not adjusted for dietary weight (same results with or without dietary weights)

**************************
* GHG/100g by Age Groups *
**************************
...
*Summarize data by food codes and GHG/100g, (don't need to adjust for dietary weight - same results for GHG intensity)
collapse (mean) mean_ghg100 = ghg100, by(food_code drxfcsd category_description foodcat_lvl1 tb_ageCat)

*Change order of columns
order tb_ageCat foodcat_lvl1 category_description

*Sort columns - descending direction for GHG/100g, ascending direction for ageCat
gsort tb_ageCat -mean_ghg100
...
*Need to convert ageCat (numeric) to string
...
export excel "Analysis_fdcodes-fdlvl3_ghg100g_by age.xlsx", firstrow(variables) keepcellfmt replace
...
foreach f of local foodcat_lvl1 {
	foreach g of local tb_ageCatstr {
		export excel using "Analysis_fdcodes-fdlvl3_ghg100g_by age.xlsx" ///
...
*Summarize data by food codes and GHG/100g, (don't need to adjust for dietary weight - same results)
collapse (mean) mean_ghg100 = ghg100, by(food_code drxfcsd category_description foodcat_lvl1 genderCat)

*Change order of columns
order genderCat foodcat_lvl1 category_description

*Sort columns - descending direction for GHG/100g, ascending direction for ageCat
gsort genderCat -mean_ghg100
...
export excel "Analysis_fdcodes-fdlvl3_ghg100g_by gender.xlsx", firstrow(variables) keepcellfmt replace
levelsof genderCatstr, local(genderCatstr)
levelsof foodcat_lvl1, local(foodcat_lvl1)

foreach f of local foodcat_lvl1 {
	foreach g of local genderCatstr {
		export excel using "Analysis_fdcodes-fdlvl3_ghg100g_by gender.xlsx" ///
...
*Summarize data by food codes and GHG/100g (don't need to adjust for dietary weight)
collapse (mean) mean_ghg100 = ghg100, by(food_code drxfcsd  category_description foodcat_lvl1 raceCat)

*Change order of columns
order raceCat foodcat_lvl1 category_description

*Sort columns - descending direction for GHG/100g, ascending direction for ageCat
gsort raceCat -mean_ghg100
...
**Export to multiple excel sheets based on two variables: Age categories and Level 1 food categories 
**2 genderCat * 5 foodcat = 10 excel sheets
export excel "Analysis_fdcodes-fdlvl3_ghg100g_by race.xlsx", firstrow(variables) keepcellfmt replace
levelsof raceCatstr, local(raceCatstr)
levelsof foodcat_lvl1, local(foodcat_lvl1)

foreach f of local foodcat_lvl1 {
	foreach g of local raceCatstr {
		export excel using "Analysis_fdcodes-fdlvl3_ghg100g_by race.xlsx" ///
